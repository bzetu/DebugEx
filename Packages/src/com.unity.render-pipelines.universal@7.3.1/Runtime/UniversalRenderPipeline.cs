...
namespace UnityEngine.Rendering.Universal
{
    public sealed partial class UniversalRenderPipeline : RenderPipeline
    {
        ...
        protected override void Render(ScriptableRenderContext renderContext, Camera[] cameras)
        {
            ...
            GraphicsSettings.lightsUseLinearIntensity = (QualitySettings.activeColorSpace == ColorSpace.Linear);//设置线性或gamma空间
        ...
        /// <!!!!!!!渲染的核心逻辑!!!!!!!>，执行culling，调用renderer的setup和execution方法
        static void RenderSingleCamera(ScriptableRenderContext context, CameraData cameraData, bool requiresBlitToBackbuffer, bool anyPostProcessingEnabled)
        {
            ...
            //设置PerCameraBuffer类中的数值，屏幕缩放比例，相机世界坐标，屏幕参数，相机矩阵
            ...
            using (new ProfilingScope(cmd, sampler))
            {
                ...
                //设置裁剪参数ScriptableCullingParameters结构体
                ...
                //填充RenderingData结构体(该结构体包含渲染相关的所有参数,如:裁剪信息，相机数据，光照阴影数据，后处理等数据)
                ...
                //调用ScriptableRenderer(默认为ForwardRenderer)的设置接口，该接口被子类重写(是扩展核心接口),传入的是默认配置的RenderingData数据
                renderer.Setup(context, ref renderingData);
                //调用ScriptableRenderer(默认为ForwardRenderer)的执行接口，该接口被子类重写(是扩展核心接口),传入的是默认配置的RenderingData数据
        ...
        {
            //读取Base相机的UniversalAdditionalCameraData数据
            ...
            int lastActiveOverlayCameraIndex = -1; //最后一个Overlay相机的index
            ...
            //初始化数据，填充Base相机的CameraData结构体,设置一系列的数值
            ...
            if (!isStackedRendering) //Base相机渲染完毕
                return;

            //处理Base相机上的所有Overlay相机,流程与Base相机差不多
            for (int i = 0; i < cameraStack.Count; ++i)
            {
